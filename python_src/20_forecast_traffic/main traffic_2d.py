...
class TrafficForeCast :
    ...
    def load_model( self, model_name ):
        ...
        model_loaded = tensorflow.keras.models.load_model( file_name )
    ...
    def train_model(self) :
        ...
        #model.add( Dense( x_dim , input_shape=(4,35) ) )

        x_layer_cnt = 2*y_dim ; y_layer_cnt = 4

        for _ in range( x_layer_cnt ) :
            model.add( Dense( x_dim ) )
        pass
        ...
        # 6. 결과 보기 
        self.test_model( model, ds["test"] )
        # // 결과 보기 

        # 7. 학습과정 살펴보기         
        history_loss = hist.history['loss']

        log.info( "max loss = %s, max_y = %s" % ( (int)(max( history_loss )), ds["train"].max_y ) )

        plt.plot( history_loss )
        plt.ylim( 0, max( history_loss ) )
        plt.ylabel( 'loss' )
        plt.xlabel( 'epoch' )
    ...
    # test model
    def test_model(self, model, ds_test ):
        print( LINE )
        ...
        print( "x dim = %s, y dim = %s" % ( x_dim, y_dim ) )
        #dim = y_list
            ...
            for r, y in enumerate( yList[ : : -1 ] ) : 
                for c, yv in enumerate( y ) : 
                    ...
                    if xv == -999.0 or xv == -999 :
                        ...
                        debug and log.info( "x[%d, %d] = %s -> %s" % ( r, c, xv, yv ) )
                    elif r == 0 :
                        y[c] = xv
...
if __name__ == '__main__':
    ...
    if "test" == mode :
        ...
        trafficForeCast.test_model( model, ds[ phase] )
    else : 
